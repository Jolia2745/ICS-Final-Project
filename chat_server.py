...
class Server:
    def __init__(self):
        ...
        self.gameranking = {15:['Helen','Amy'], 14:['Mia']}
...
# ==============================================================================
            ...
            elif msg['action'] == 'encrypted_exchange':     #!! 
                from_name = self.logged_sock2name[from_sock]
                the_guys = self.group.list_me(from_name)
                #said = msg["from"]+msg["message"]
                said2 = text_proc(msg["message"], from_name)
                ...
                for g in the_guys[1:]:
                    ...
                    #self.indices[g].add_msg_and_index(said2)
                    mysend(to_sock, json.dumps(
                        {"action": "encrypted_exchange", "from": msg["from"], "message": msg["message"]}))
...
# ==============================================================================
            ...
            elif msg["action"] == "sendscore":
                from_name = self.logged_sock2name[from_sock]
                the_guys = self.group.list_me(from_name)
                print(from_name)
                score = msg["score"]
                print("received score is:" , score)
                if score in self.gameranking.keys():
                    if from_name not in self.gameranking[score]:
                        self.gameranking[score].append(from_name)
                else:
                    self.gameranking[score] = [from_name]
                revl = sorted(self.gameranking, reverse = True)
                ranking = {}
                for item in revl:
                    ranking[item] = self.gameranking[item]
                self.gameranking = ranking



                print("final output is:" , self.gameranking)
            
            elif msg["action"] == "getscore":
                from_name = self.logged_sock2name[from_sock]
                mysend(from_sock, json.dumps({"action": "getscore", "results": self.gameranking}))
